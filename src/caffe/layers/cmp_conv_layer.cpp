...
void CmpConvolutionLayer<Dtype>::ComputeBlobMask() {
  ...
  int index = static_cast<int>(count * ratio);
  ...
  if (index > 0) {
    ...
    LOG(INFO) << this->layer_param_.name() << " threshold and ratio: " << threshold << " " << ratio << endl;
    for (int i = 0; i < count; ++i) {
  ...
  if (this->quantize_term_) {
    ...
    kmeans_cluster(this->indices_.mutable_cpu_data(), this->centroids_.mutable_cpu_data(), muweight, count,
                   mask_data/*this->masks_*/,/* max_weight, min_weight,*/ nCentroid, 1000);
  }
}
